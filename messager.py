...
class ShowInfo():
    def __init__(self, msg_info):
        ...
        table_msg = gtk.Table(10, 2, False)
        ...
        table_msg.attach(label_last_played0, 0, 1, 9, 10, True, True, 5, 5)
        table_msg.attach(label_last_played1, 1, 2, 9, 10, True, True, 5, 5)
    ...
    def last_played(self, code):
        query = "select when_played from playlog where id_type='msg' AND id_code='{0}' LIMIT 1".format(code)
...
class ChangeMessage():
    def __init__(self, msg_info):
        ...
        table_msg.attach(hbox_filename, 0, 2, 1, 2, True, True, 0, 0)
        table_msg.attach(label_title, 0, 1, 2, 3, True, True, 0, 0)
        table_msg.attach(self.entry_title, 1, 2, 2, 3, True, True, 0, 0)
        table_msg.attach(label_nq, 0, 1, 3, 4, True, True, 0, 0)
        table_msg.attach(self.entry_nq, 1, 2, 3, 4, True, True, 0, 0)
        table_msg.attach(label_expiry, 0, 1, 4, 5, True, True, 0, 0)
        table_msg.attach(self.entry_expiry, 1, 2, 4, 5, True, True, 0, 0)
        table_msg.attach(label_prod, 0, 1, 5, 6, True, True, 0, 0)
        table_msg.attach(self.entry_prod, 1, 2, 5, 6, True, True, 0, 0)
        table_msg.attach(btn_change, 0, 2, 6, 7, True, True, 5, 5)
...
class NewMessage:
    ...
    def add_message(self, widget):
        ...
        self.add_to_db(msg_type, title, code, nq, expiry_date, filename, producer, dur)
        self.dialog_added()
        self.clear_fields()
    ...
    def add_to_db(self, msg_type, title, code, nq, expiry_date, filename, producer, dur):
        col_items = '(code, title, type, nq, expirydate, filename, fldproducer, duration)'
        ...
        val_items = (code, title, msg_type, nq, expiry_date, filename, producer, dur)
...
class Messager():
    ...
    def main(self):
        ...
        #make the list
        self.msg_store = gtk.ListStore(str ,str ,str ,str ,str ,str ,str, str, str)
    ...
    def get_selection(self):
        treeselection = self.treeview_msg.get_selection()
        model, iter = treeselection.get_selected()
        msg_info = model.get(iter, 0, 1, 2, 3, 4, 5, 6, 7)
        return msg_info
